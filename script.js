// FINAL OPTIMIZED script.js - All INP Improvements + No Long Tasks
...
// Initialize mobile menu when DOM is ready (CRITICAL - don't defer)
...
// Cache nav height to prevent forced reflow - PERFORMANCE FIX
...
// Smooth scrolling for navigation links (optimized - no forced reflow)
// CRITICAL: This must work immediately, so init right away but use lightweight approach
...
// Initialize smooth scrolling immediately (small, fast operation)
if (document.readyState === 'loading') {
...
// Defer all non-critical features to avoid blocking main thread
...
const initScrollAnimations = () => {
    ...
    const observerOptions = {
        threshold: 0.1,
        rootMargin: '0px 0px -50px 0px' // Reduced from -100px for earlier triggering
    };
        ...
        requestAnimationFrame(() => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    // Use transform instead of direct style manipulation to avoid reflow
...
// Cache viewport height and update only when needed
let cachedVh = 0;
const updateViewportHeight = () => {
    const newVh = window.innerHeight * 0.01;
    // Only update if changed significantly (more than 1px difference)
    if (Math.abs(newVh - cachedVh) > 0.01) {
        cachedVh = newVh;
        requestAnimationFrame(() => {
...
const handleOrientationChange = debounce(updateViewportHeight, 100);

window.addEventListener('orientationchange', handleOrientationChange, { passive: true });
window.addEventListener('resize', debounce(updateViewportHeight, 250), { passive: true });
...
// Use requestIdleCallback to chunk work and avoid long main-thread tasks
...
const initializeNonCriticalFeatures = () => {
    ...
    // Other non-critical features initialize here
    // (dots rotation, newsletter, skill bars, etc. already handle themselves)
};

// Use requestIdleCallback to avoid blocking main thread
// This prevents long tasks and improves TBT (Total Blocking Time)
if ('requestIdleCallback' in window) {
    requestIdleCallback(initializeNonCriticalFeatures, { timeout: 2000 });
} else {
...
window.addEventListener('beforeunload', () => {
    ...
    if (typeof scrollObserver !== 'undefined' && scrollObserver) {
